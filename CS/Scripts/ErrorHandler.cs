...
public class ErrorHandler {
  ...
  public static void SetLogPath() {
    ...
    string logFileName =
        System.IO.Path.Combine(Properties.Settings.Default.App_PathLocalData,
                               AssemblyInfo.Product + ".log");
  ...
  public static void CreateLogRecord() {
    try {
      ...
      var logMessage = string.Concat(
          new Dictionary<string, string> { ["PROCEDURE"] = currentProcedure,
  ...
  /// </example>
  /// </summary>
  /// <param name="ex">Represents errors that occur during application
  /// execution.</param> <param name="isSilent">Used to show a message to the
  /// user and log an error record or just log a record.</param>
  /// <remarks></remarks>
  public static void DisplayMessage(Exception ex, Boolean isSilent = false) {
    ...
    // handle message
    if (isSilent == false) {
      MessageBox.Show(userMessage, "Unexpected Error", MessageBoxButtons.OK,
  ...
  /// <returns>A method that returns true or false if there is an active
  /// document </returns> <remarks></remarks>
  public static bool IsActiveDocument(bool showMsg = false) {
    try {
      if (Globals.ThisAddIn.Application.ActiveWorkbook == null) {
        if (showMsg == true) {
          MessageBox.Show(
              "The command could not be completed.  Please open a document and select a range.",
              AssemblyInfo.Description, MessageBoxButtons.OK,
  ...
  /// <param name="showMsg">To show a message </param>
  /// <returns>A method that returns true or false if there is an active
  /// selection </returns> <remarks></remarks>
  public static bool IsActiveSelection(bool showMsg = false) {
    ...
    try {
      checkRange = Globals.ThisAddIn.Application.Selection as Excel
                       .Range; // must cast the selection as range or errors
      if (null == checkRange) {
        if (showMsg == true) {
          MessageBox.Show(
              "The command could not be completed by using the range specified.  Select a single cell within the range and try the command again. [Range]",
              AssemblyInfo.Description, MessageBoxButtons.OK,
              MessageBoxIcon.Exclamation);
        }
        return false;
  ...
  /// </summary>
  /// <param name="showMsg">To show a message </param>
  /// <returns>A method that returns true or false if there is a valid list
  /// object </returns> <remarks></remarks>
  public static bool IsValidListObject(bool showMsg = false) {
    Excel.ListObject tbl = null;
    try {
      tbl = Globals.ThisAddIn.Application.ActiveCell
                .ListObject; // directly after the table is created this is not
                             // true
      if ((tbl == null)) {
        if (showMsg == true) {
          MessageBox.Show(
              "The command could not be completed by using the range specified.  Select a single cell within the range and try the command again. [ListObject]",
              AssemblyInfo.Description, MessageBoxButtons.OK,
              MessageBoxIcon.Exclamation);
        }
        return false;
  ...
  /// <returns></returns>
  public static bool IsValidListColumn(Excel.ListObject tbl,
                                       string columnName) {
    try {
  ...
  /// There are few ways to check this (eg. check to see if a standard menu item
  /// is disabled etc.) I know in cell editing mode app.DisplayAlerts throws an
  /// Exception, so here I'm relying on that behaviour
  /// </summary>
  /// <param name="showMsg">To show a message </param>
  ...
  private static bool IsInCellEditingMode(bool showMsg = false) {
    bool flag = false;
    try {
      Globals.ThisAddIn.Application.DisplayAlerts =
          false; // This will throw an Exception if Excel is in Cell Editing
                 // Mode
    } catch (Exception) {
      if (showMsg == true) {
        MessageBox.Show(
            "The procedure can not run while you are editing a cell.",
            "No action taken.", MessageBoxButtons.OK,
  ...
  /// </summary>
  /// <param name="showMsg">To show a message </param>
  /// <returns>A method that returns true or false if an object can be enabled
  ...
  /// </summary>
  /// <param name="showMsg">To show a message </param>
  /// <returns>A method that returns true or false if an object can be used
  /// </returns> <remarks></remarks>
  ...
  /// <returns>A method that returns true or false if there is a valid date
  /// </returns> <remarks></remarks>
  ...
  /// </summary>
  /// <param name="expression">Represents the cell value </param>
  /// <returns>A method that returns true or false if there is a valid time
  /// </returns> <remarks></remarks>
  public static bool IsTime(object expression) {
    try {
      string timeValue = Convert.ToString(expression);
      // timeValue = String.Format("{0:" +
      // Properties.Settings.Default.Table_ColumnFormatTime + "}", expression);
      // timeValue =
      // timeValue.ToString(Properties.Settings.Default.Table_ColumnFormatTime,
      // System.Globalization.CultureInfo.InvariantCulture); timeValue =
      // timeValue.ToString(Properties.Settings.Default.Table_ColumnFormatTime);
      // string timeValue =
      // expression.ToString().Format(Properties.Settings.Default.Table_ColumnFormatTime);
      TimeSpan time1;
