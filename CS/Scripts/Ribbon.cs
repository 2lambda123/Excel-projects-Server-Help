...
#region | Ribbon Events |
  ...
  /// <summary>
  /// Loads the XML markup, either from an XML customization file or from XML
  /// markup embedded in the procedure, that customizes the Ribbon user
  /// interface.
  /// </summary>
  /// <param name="ribbonID">Represents the XML customization file </param>
  /// <returns>A method that returns a bitmap image for the control id.
  ...
  /// <summary>
  /// Called by the GetCustomUI method to obtain the contents of the Ribbon XML
  /// file.
  /// </summary>
  ...
  /// </summary>
  /// <param name="ribbonUI">Represents the IRibbonUI instance that is provided
  /// by the Microsoft Office application to the Ribbon extensibility code.
  ...
  /// </summary>
  /// <param name="control">Represents the object passed into the callback
  /// procedure of a control in a ribbon or another user interface that can be
  /// customized by using Office Fluent ribbon extensibility. </param>
  /// <returns>A method that returns a bitmap image for the control id.
  /// </returns>
  ...
  public string GetLabelText(Office.IRibbonControl control) {
    try {
      ...
      case "tabServerActions":
        if (Application.ProductVersion.Substring(0, 2) ==
            "15") // for Excel 2013
        {
      ...
      case "txtDescription":
        return AssemblyInfo.Title.Replace("&", "&&") + " " +
  ...
  /// procedure of a control in a ribbon or another user interface that can be
  /// customized by using Office Fluent ribbon extensibility. </param>
  /// <returns>A method that returns an integer of total count of items used for
  /// a combobox or dropdown </returns>
  ...
  /// <param name="control">Represents the object passed into the callback
  /// procedure of a control in a ribbon or another user interface that can be
  /// customized by using Office Fluent ribbon extensibility. </param> <param
  /// name="index">Represents the index of the combobox or dropdown value
  /// </param> <returns>A method that returns a string per index of a combobox
  /// or dropdown </returns>
  ...
  /// procedure of a control in a ribbon or another user interface that can be
  /// customized by using Office Fluent ribbon extensibility. </param>
  /// <returns>A method that returns a string for the default value of a
  /// combobox </returns>
  ...
  /// procedure of a control in a ribbon or another user interface that can be
  /// customized by using Office Fluent ribbon extensibility. </param>
  /// <returns>A method that returns true or false if the control is enabled
  /// </returns>
  ...
  /// procedure of a control in a ribbon or another user interface that can be
  /// customized by using Office Fluent ribbon extensibility. </param> <param
  /// name="text">Represents the text from the combobox value </param>
...
#region | Ribbon Buttons |
    ...
    try {
      ...
      if (ErrorHandler.IsValidListObject()) {
        Excel.ListObject tbl =
            Globals.ThisAddIn.Application.ActiveCell.ListObject;

        if (ErrorHandler.IsValidListColumn(
                tbl, Properties.Settings.Default.Ping_Results)) {
          colResults =
              tbl.ListColumns[Properties.Settings.Default.Ping_Results];
        } else {
        ...
        if (ErrorHandler.IsValidListColumn(
                tbl, Properties.Settings.Default.Ping_ServerName)) {
          Excel.ListColumn colServer =
              tbl.ListColumns[Properties.Settings.Default.Ping_ServerName];
          for (int r = 1; r <= tbl.ListRows.Count; r++) {
            if (colServer.DataBodyRange.Rows[r].EntireRow.Hidden == false) {
              Excel.Range cellResult =
                  colResults.DataBodyRange.Cells[1].Offset(r - 1, 0);
              Excel.Range cellServer =
                  colServer.DataBodyRange.Cells[1].Offset(r - 1, 0);
              cellResult.Value =
                  Ribbon.GetPingResult(cellServer.Value.ToString());
            }
          }
        }
      }
  ...
  public void CreateRdgFile() {
    try {
      ...
      if (ErrorHandler.IsValidListObject()) {
        var script =
            new StringBuilder()
                .AppendLine("<?xml version=" + quote + "1.0" + quote +
                            " encoding=" + quote + "UTF-8" + quote + "?>")
                .AppendLine("<RDCMan programVersion=" + quote + "2.7" + quote +
                            " schemaVersion=" + quote + "3" + quote + ">")
                .AppendLine("<file>")
        ...
        Excel.ListObject tbl =
            Globals.ThisAddIn.Application.ActiveCell.ListObject;
        Excel.ListColumn colServer =
            tbl.ListColumns[Properties.Settings.Default.Rdg_ServerName];
        Excel.ListColumn colDesc =
            tbl.ListColumns[Properties.Settings.Default.Rdg_Description];
        Excel.ListColumn colComment =
            tbl.ListColumns[Properties.Settings.Default.Rdg_Comment];
        Excel.ListColumn colServerGroup =
            tbl.ListColumns[Properties.Settings.Default.Rdg_ServerGroup];
        ...
        for (int r = 1; r <= tbl.ListRows.Count; r++) {
          if (colServer.DataBodyRange.Rows[r].EntireRow.Hidden == false) {
            ...
            Excel.Range cellDesc =
                colDesc.DataBodyRange.Cells[1].Offset(r - 1, 0);
            Excel.Range cellComment =
                colComment.DataBodyRange.Cells[1].Offset(r - 1, 0);
            Excel.Range cellServerGroup =
                colServerGroup.DataBodyRange.Cells[1].Offset(r - 1, 0);
            ...
            script.AppendLine("<displayName>" + cellServer.Value + " (" +
                              cellDesc.Value + ")</displayName>");
            script.AppendLine("<comment>" & cellComment.Value & "</comment>");
            script.AppendLine("</properties>");
            script.AppendLine("</server>");

            if (currentGroup !=
                colServerGroup.DataBodyRange.Cells[1].Offset(r, 0)) {
              script.AppendLine("</group>");
            }
          }
        ...
        System.IO.File.WriteAllText(Properties.Settings.Default.Rdg_FileName,
                                    script.ToString());
      }
    } catch (Exception ex) {
      ErrorHandler.DisplayMessage(ex);
  ...
  public void RefreshServerList() {
    ...
    try {
      ...
      ldapQry = ldapQry.Replace("[Rdg.LdapPath]",
                                Properties.Settings.Default.Rdg_LdapPath);
      ...
      //     string msg = "The sheet named '" & My.Settings.Rdg_SheetName & "'
      //     does not exist." msg = msg & vbCrLf & "Would you like to use the
      //     current sheet?" answer = MsgBox(msg, vbYesNo + vbQuestion, "Sheet
      //     Not Found")
      //   'MessageBox.Show(msg, "Unexpected Error", MessageBoxButtons.OK,
      ...
      //'create server type column from the first 2 characters of the server
      //name 'If (My.Settings.Rdg_ServerGroup = "ServerType" Then '
      //tbl.ListColumns.Add(3).Name = My.Settings.Rdg_ServerGroup '
      //tbl.ListColumns(My.Settings.Rdg_ServerGroup).DataBodyRange.FormulaR1C1 =
      //"=UPPER(IFERROR(IF(SEARCH(""-"", [@Name]) > 0, LEFT([@Name], 2), """"),
      //""(NONE)""))" '    Globals.ThisAddIn.Application.Columns.AutoFit() 'End
      //If
  ...
  public void OpenSettings() {
    try {
      if (myTaskPaneSettings != null) {
        ...
        myTaskPaneSettings = Globals.ThisAddIn.CustomTaskPanes.Add(
            mySettings, "Settings for " + Scripts.AssemblyInfo.Title);
        myTaskPaneSettings.DockPosition =
            Office.MsoCTPDockPosition.msoCTPDockPositionRight;
        myTaskPaneSettings.DockPositionRestrict =
  ...
  public void OpenReadMe() {
    try {
      ...
      System.Diagnostics.Process.Start(
          Properties.Settings.Default.App_PathReadMe);
    } catch (Exception ex) {
      ErrorHandler.DisplayMessage(ex);
    }
  }
  ...
  public void OpenNewIssue() {
    try {
      ErrorHandler.CreateLogRecord();
      System.Diagnostics.Process.Start(
          Properties.Settings.Default.App_PathNewIssue);
    } catch (Exception ex) {
      ErrorHandler.DisplayMessage(ex);
    }
  }
...
#region | Subroutines |
  ...
  /// </summary>
  public void InvalidateRibbon() { ribbon.Invalidate(); }
  ...
  public static string GetPingResult(string hostName) {
    ...
    try {
      SelectQuery query =
          new SelectQuery("Win32_PingStatus", "Address='" + hostName + "'");
      ...
      foreach (ManagementObject result in searcher.Get()) {
        if (result["StatusCode"] != null) {
          ...
          case 11001: {
            resultText = "Buffer too small";
            break;
          }
          case 11002: {
            resultText = "Destination net unreachable";
            break;
          }
          case 11003: {
            resultText = "Destination host unreachable";
            break;
          }
          case 11004: {
            resultText = "Destination protocol unreachable";
            break;
          }
          case 11005: {
            resultText = "Destination port unreachable";
            break;
          }
          case 11006: {
            resultText = "No resources";
            break;
          }
          case 11007: {
            resultText = "Bad option";
            break;
          }
          case 11008: {
            resultText = "Hardware error";
            break;
          }
          case 11009: {
            resultText = "Packet too big";
            break;
          }
          case 11011: {
          ...
          case 11012: {
            resultText = "Bad route";
            break;
          }
          case 11013: {
            resultText = "Time-To-Live (TTL) expired transit";
            break;
          }
          case 11014: {
            resultText = "Time-To-Live (TTL) expired reassembly";
            break;
          }
          case 11015: {
            resultText = "Parameter problem";
            break;
          }
          case 11016: {
            resultText = "Source quench";
            break;
          }
          case 11017: {
            resultText = "Option too big";
            break;
          }
          case 11018: {
            resultText = "Bad destination";
            break;
          }
          case 11032: {
            resultText = "Negotiating IPSEC";
            break;
          }
          case 11050: {
            resultText = "General failure";
            break;
          }
          default: {
  ...
  /// <summary>
  /// Finds dates columns with the paste format settings or date specific
  /// columns and updates with date format setting
  /// </summary>
  /// <remarks></remarks>
  ...
  public static void FormatTableFromRange() {
    ...
    string tableName = AssemblyInfo.Title + " " +
    ...
    try {
      ...
      // create the table
      Excel.Range rng = Globals.ThisAddIn.Application.ActiveSheet.Range(
          Globals.ThisAddIn.Application.ActiveSheet.Range(FirstCellName),
          Globals.ThisAddIn.Application.ActiveSheet.Range(FirstCellName)
              .SpecialCells(Excel.Constants.xlLastCell));
      rng.Worksheet.ListObjects
          .Add(Excel.XlListObjectSourceType.xlSrcRange, rng,
               System.Type.Missing, Excel.XlYesNoGuess.xlYes,
               System.Type.Missing)
          .Name = tableName;
      ...
      Globals.ThisAddIn.Application.ActiveSheet.Application.ActiveWindow
          .FreezePanes = false;
      Globals.ThisAddIn.Application.ActiveSheet.Application.ActiveWindow
          .ScrollRow = 1;
      Globals.ThisAddIn.Application.ActiveSheet.Application.ActiveWindow
